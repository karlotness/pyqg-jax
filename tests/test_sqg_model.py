...
def test_default_parameters_match(param):
    ...
    jax_model = pyqg_jax.sqg_model.SQGModel(precision=pyqg_jax.state.Precision.DOUBLE)
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=DeprecationWarning)
        orig_model = pyqg.SQGModel(log_level=0)
...
def test_match_final_step(precision):
    pyqg = pytest.importorskip("pyqg")
    jax_model = pyqg_jax.sqg_model.SQGModel(precision=precision, **SQG_PARAMS)
    start_jax_state = jax_model.create_initial_state(jax.random.key(0))
    dt = 0.005
    num_steps = 1000
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", category=DeprecationWarning)
        orig_model = pyqg.SQGModel(
            log_level=0,
            dt=dt,
            tmax=dt * num_steps,
            twrite=num_steps + 10,
    ...
    assert jnp.all(
        relerr < (0.005 if precision == pyqg_jax.state.Precision.SINGLE else 1e-10)
    )
...
def test_rectangular_stepping(nx, ny):
    jax_model = pyqg_jax.steppers.SteppedModel(
        model=pyqg_jax.sqg_model.SQGModel(**(SQG_PARAMS | {"nx": nx, "ny": ny})),
        stepper=pyqg_jax.steppers.AB3Stepper(dt=0.005),
    )
...
def test_tree_flatten_roundtrip():
    model = pyqg_jax.sqg_model.SQGModel(**SQG_PARAMS)
